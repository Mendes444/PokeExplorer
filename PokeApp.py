...
# SCREEN 4: TEAM BUILDER SCREEN
    ...
    # 1. Display the Team Roster (Always 6 spots)
    team_cols = st.columns(6)
    
    for idx in range(6):
        with team_cols[idx]:
            with st.container(border=True):
                # If there is a Pokémon in this slot
                if idx < len(st.session_state.team):
                    t_name = st.session_state.team[idx]
                ...
                # If the slot is empty
                    ...
                    # Show a Pokéball placeholder and an Add button
                    st.image("https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/poke-ball.png", use_container_width=True)
                    st.markdown("<center><b>Empty Slot</b></center>", unsafe_allow_html=True)
                    st.button("➕ Add", key=f"add_empty_{idx}", on_click=go_home_cb, use_container_width=True)
